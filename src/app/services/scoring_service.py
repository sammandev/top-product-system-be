...
def detect_scoring_type(test_item: dict) -> ScoringType:
    ...
    Rules:
    1. Binary (PASS/FAIL value) -> BINARY
    2. Only UCL, typically negative values -> EVM
    3. Only LCL (lower limit only) -> THROUGHPUT
    4. UCL and LCL, symmetric -> SYMMETRICAL
    5. Only UCL, values near 0 -> PER_MASK
    """
...
def score_symmetrical_nl(
    value: float,
    ucl: float,
    lcl: float,
    target_score: float = 0.8,
    target_deviation: float = 2.5
) -> tuple[float, float]:
    """
...
def score_record(
    ...
    include_binary_in_overall: bool = True
) -> RecordScoreResult:
    """
...
def calculate_scores(request: CalculateScoresRequest) -> CalculateScoresResponse:
    ...
    config_map: dict[str, ScoringConfig] = {
        config.test_item_name: config for config in request.scoring_configs
    }
