...
async def calculate_test_scores(
    ...
    try:
        logger.info(
            f"User {current_user.username} calculating scores for {len(request.records)} records"
        )
        
        result = calculate_scores(request)
        
        logger.info(
            f"Scored {len(result.scored_records)} records, "
            f"average: {result.summary.average_score:.3f}"
        )
...
async def detect_scoring_types(
    record: dict,
    current_user: Annotated[UserRead, Depends(get_current_user)]
) -> list[dict]:
    ...
    try:
        ...
        for item in test_items:
            ...
            if name and name not in seen_names:
                ...
                results.append({
                    "test_item_name": name,
                    "detected_type": scoring_type.value,
                    "value": item.get("VALUE", ""),
                    "ucl": item.get("UCL", ""),
                    "lcl": item.get("LCL", ""),
                    "default_params": SCORING_TYPE_DEFAULTS.get(scoring_type, {})
                })
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error detecting scoring types: {str(e)}"
        )
...
@router.get(
    "/types",
    response_model=list[dict],
    summary="Get available scoring types",
    description="Returns list of available scoring types with descriptions and default parameters."
)
...
async def preview_score(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Error calculating preview: {str(e)}"
        )
